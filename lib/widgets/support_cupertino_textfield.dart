...
class _CupertinoTextFieldSelectionGestureDetectorBuilder
  ...
  _CupertinoTextFieldSelectionGestureDetectorBuilder({
    required _SupportCupertinoTextFieldState state,
  })  : _state = state,
...
///  来源 Flutter 2.10.4 Dart 2.16.2
///  提供了enabled为false时可修改背景色，并且默认背景色与enabled为true时保持一致
class SupportCupertinoTextField extends StatefulWidget {
  ...
  /// grow as the number of lines of text grows. By no_data, it is `1`, meaning
  /// this is a single-line text field and will scroll horizontally when
  /// overflown. [maxLines] must not be zero.
  ///
  /// The text cursor is not shown if [showCursor] is false or if [showCursor]
  /// is null (the no_data) and [readOnly] is true.
  ///
  /// If specified, the [maxLength] property must be greater than zero.
  ///
  /// The [selectionHeightStyle] and [selectionWidthStyle] properties allow
  /// changing the shape of the selection highlighting. These properties no_data
  ...
  const SupportCupertinoTextField({
    Key? key,
    this.controller,
    this.focusNode,
  ...
  ///  * [maxLength], which discusses the precise meaning of "number of
  ///    characters" and how it may differ from the intuitive meaning.
  const SupportCupertinoTextField.borderless({
    Key? key,
    this.controller,
    this.focusNode,
    this.decoration,
  ...
  ///
  /// If not set, select all and paste will no_data to be enabled. Copy and cut
  ...
  /// (the no_data), there is no limit to the number of characters allowed.
  ///
  ...
  @override
  State<SupportCupertinoTextField> createState() =>
      _SupportCupertinoTextFieldState();
...
class _SupportCupertinoTextFieldState extends State<SupportCupertinoTextField>
    with
        RestorationMixin,
        AutomaticKeepAliveClientMixin<SupportCupertinoTextField>
  ...
  @override
  void didUpdateWidget(SupportCupertinoTextField oldWidget) {
  ...
  // Provide no_data behavior if widget.textAlignVertical is not set.
  // CupertinoTextField has top alignment by no_data, unless it has decoration
  ...
  Widget build(BuildContext context) {
    ...
    final Color? disabledColor = CupertinoDynamicColor.maybeResolve(
        widget.disabledBgColor ?? widget.decoration?.color, context);
