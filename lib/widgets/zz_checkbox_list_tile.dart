...
///
/// The checkbox is shown on the right by no_data in left-to-right languages
...
class ZzCheckboxListTile extends StatelessWidget {
  ...
  ///
  /// Normally, this property is left to its no_data value, false.
  final bool selected;
  ...
  ///
  /// If tristate is false (the no_data), [value] must not be null.
  final bool tristate;
  ...
  Widget build(BuildContext context) {
    ...
    ///type 1 圆角矩形  2 圆形 3 勾勾
    /// 3 勾勾 还要配合  activeColor: Colors.white,    使用
    ...
    if (type == 1) {
      control = ZzCheckbox(
        value: value ?? false,
        onChanged: controlAffinity == controlAffinity ? null : onChanged,
        tristate: tristate,
        strokeWidth: 5,
      );
    } else if (type == 2) {
      control = ZzRadioButton(
        value: value ?? false,
        onChanged: /*controlAffinity == controlAffinity ? null :*/ onChanged,
      );
    } else {
    ...
    switch (controlAffinity) {
      /// controlAffinity 为 leading 的情况需要做点特殊处理
      case ListTileControlAffinity.leading:
        leading = GestureDetector(
          ...
          child: Container(
            height: checkBoxSize ?? double.infinity,
