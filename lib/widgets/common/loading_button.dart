...
class RoundedLoadingButton extends StatefulWidget {
  ...
  const RoundedLoadingButton({
    ...
    this.completionDuration = const Duration(milliseconds: 300),
    this.disabledColor,
    this.noStyle,
  }) : super(key: key);
...
class RoundedLoadingButtonState extends State<RoundedLoadingButton>
    ...
    final _btn = ButtonTheme(
      ...
      child: ElevatedButton(
        style: ElevatedButton.styleFrom(
            ...
            shape: (widget.noStyle ?? false)
                ? null
                : RoundedRectangleBorder(
                    borderRadius: BorderRadius.circular(widget.borderRadius),
                  ),
            //   primary: widget.color,
            elevation: (widget.noStyle ?? false) ? 0 : widget.elevation,
            backgroundColor: Colors.transparent,
            padding: const EdgeInsets.all(0),
            side: (widget.noStyle ?? false)
                ? null
                : const BorderSide(color: ColorName.themeColor)),
  ...
  void initState() {
    ...
    _squeezeAnimation =
        Tween<double>(begin: widget.width, end: widget.width).animate(
